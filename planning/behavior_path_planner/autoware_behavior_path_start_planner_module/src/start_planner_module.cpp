...
void StartPlannerModule::updatePullOutStatus()
{
  ...
  const std::vector<Pose> start_pose_candidates = std::invoke([&]() -> std::vector<Pose> {
    if (parameters_->enable_back) {
      ...
      // Remove the first candidate from searchPullOutStartPoseCandidates (back_distance=0.0,
      // equivalent to current position) Note: The remaining candidates yaw are assumed to be
      // aligned along the lane yaw after back driving
      if (!candidates.empty()) {
