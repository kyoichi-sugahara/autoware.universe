  ...
  const std::shared_ptr<const PlannerData> & planner_data, PlannerDebugData & planner_debug_data)
{
  ...
  const double lateral_offset =
    autoware::motion_utils::calcLateralOffset(road_lanes, start_pose.position);
...
bool ClothoidPullOut::refineShiftedPathToStartPose(
  ...
  const double longitudinal_acc, const double lateral_acc)
{
  ...
  while (iteration < MAX_ITERATION) {
    ...
    if (!path_shifter.generate(&shifted_path, false)) {
      RCLCPP_WARN(
        rclcpp::get_logger("ClothoidPullOut:refineShiftedPathToStartPose()"),
...
std::vector<PullOutPath> ClothoidPullOut::calcPullOutPaths(
...
double ClothoidPullOut::calcPullOutLongitudinalDistance(
...
double ClothoidPullOut::calcBeforeShiftedArcLength(
