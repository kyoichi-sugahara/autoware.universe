...
protected:
  ...
  std::vector<lanelet::ConstLanelet> return_lanelets(
    const double center_x, const double center_y, const double length_x, const double width_y)
  {
    lanelet::LineString3d left_bound;
    lanelet::LineString3d right_bound;
    lanelet::LineString3d centerline;
    left_bound.push_back(
      lanelet::Point3d{lanelet::InvalId, center_x - length_x, center_y + width_y});
...
TEST_F(TestGeometricParallelParking, GenerateValidClothoidPullOverPath)
{
  ...
  auto paths_forward_left = geometric_parallel_parking_.planOneTrialClothoid(
    start_pose, goal_pose, R_E_far_, L_min, road_lanes, left_shoulder_lanes, true, true, 0.0, 0.0,
    arc_path_interval_, lane_departure_checker_);

  goal_pose.position.x = 3.0;
  goal_pose.position.y = -1.0;
  auto paths_forward_right = geometric_parallel_parking_.planOneTrialClothoid(
    start_pose, goal_pose, R_E_far_, L_min, road_lanes, right_shoulder_lanes, true, false, 0.0, 0.0,
    arc_path_interval_, lane_departure_checker_);

  // Assert that a valid geometric pull out path is generated
  EXPECT_GT(paths_forward_left.size(), 0);
  EXPECT_GT(paths_forward_right.size(), 0);
}
