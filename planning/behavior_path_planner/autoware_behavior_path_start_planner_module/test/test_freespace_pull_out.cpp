...
protected:
  void SetUp() override
  {
    ...
    node_ = rclcpp::Node::make_shared("freespace_pull_out", get_node_options());
...
private:
  ...
  void freespace_pull_out_planner()
  {
    ...
    parameters->vehicle_shape_margin = 0.0;
    parameters->freespace_planner_algorithm = "astar";
    parameters->astar_parameters.use_back = true;

    auto time_keeper = std::make_shared<autoware::universe_utils::TimeKeeper>();
    freespace_pull_out_ =
      std::make_shared<FreespacePullOut>(*node_, *parameters, vehicle_info_, time_keeper);
  }
  ...
  void initialize_planner_data()
  {
    ...
    costmap_.info.origin =
      geometry_msgs::build<geometry_msgs::msg::Pose>()
        .position(
          geometry_msgs::build<geometry_msgs::msg::Point>().x(362.181).y(362.164).z(100.000))
        .orientation(
          geometry_msgs::build<geometry_msgs::msg::Quaternion>().x(0.0).y(0.0).z(0.709650).w(
            0.704554));
    planner_data_.costmap = std::make_shared<nav_msgs::msg::OccupancyGrid>(costmap_);
  }
  ...
  std::string freespace_planner_algorithm_{""};
...
TEST_F(TestFreespacePullOut, DISABLED_GenerateValidFreespacePullOutPath)
{
  ...
  freespace_pull_out_->setPlannerData(std::make_shared<PlannerData>(planner_data_));
