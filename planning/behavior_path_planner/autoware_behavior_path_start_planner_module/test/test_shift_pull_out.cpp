...
public:
  ...
  {
    return shift_pull_out->calcPullOutLongitudinalDistance(
  ...
  std::shared_ptr<ShiftPullOut> shift_pull_out;
...
private:
  void init_param()
  {
    ...
    auto node = rclcpp::Node::make_shared("test_shift_pull_out", node_options);
    ...
    shift_pull_out =
      std::make_shared<ShiftPullOut>(*node, *parameters, lane_departure_checker, time_keeper);
  }
  ...
  void init_module()
  {
    // shift_pull_out = std::make_shared<ShiftPullOut>(
...
TEST_F(TestShiftPullOut, calcPullOutLongitudinalDistance)
{
  ...
  calcPullOutLongitudinalDistance(lon_acc, shift_time, shift_length, max_curvature, min_distance);
}
}  // namespace autoware::behavior_path_planner
